...
void TagTrackerPulseDisplayTest::_bearingResultValidity_data()
{
    ...
    QTest::newRow("finite, no slices")      << 123.4f << 0u << true;
...
void TagTrackerPulseDisplayTest::_bearingResultValidity()
{
    ...
    const bool expectInconsistencyWarning = expectedValid && nValidSlices == 0;

    if (expectInconsistencyWarning) {
        expectLogMessage("CustomPluginLog", QtWarningMsg, QRegularExpression(QStringLiteral("n_valid_slices == 0")));
    }
...
void TagTrackerPulseDisplayTest::_bearingState()
{
    ...
    QCOMPARE(waiting.bearingState(), RotationInfo::Pending);
    QVERIFY(waiting.bearingStateText().isEmpty());
    ...
    // Detections attributed to the tag but no bearing: a lock that did not fit
    // the pattern, or sub-lock hits agreeing in frequency across headings.
    RotationInfo heard(8);
    heard.setBearingResult(nan, 0.0f, 3u, 20.7f, false);
    QVERIFY(heard.bearingHeard());
    QVERIFY(!heard.bearingValid());
    QCOMPARE(heard.bearingState(), RotationInfo::Heard);
    QCOMPARE(heard.bearingStateText(), QStringLiteral("heard, no bearing"));
    QCOMPARE(heard.bearingSector(), -1);
    QVERIFY(!nothing.bearingHeard());
...
void TagTrackerPulseDisplayTest::_outcomeAnnouncement()
{
    ...
    RotationInfo heard(8);
    heard.setBearingResult(nan, 0.0f, 3u, 20.7f, false);
    QCOMPARE(PythonRotateAndCaptureState::outcomeAnnouncement(&heard),
             QStringLiteral("Rotation complete. Tag heard, no bearing"));
