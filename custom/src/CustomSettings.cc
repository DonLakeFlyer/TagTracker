/****************************************************************************
 *
 *   (c) 2009-2016 QGROUNDCONTROL PROJECT <http://www.qgroundcontrol.org>
 *
 * QGroundControl is licensed according to the terms in the file
 * COPYING.md in the root of the source code directory.
 *
 ****************************************************************************/

#include "CustomSettings.h"

#include <QQmlEngine>
#include <QtQml>

DECLARE_SETTINGGROUP(Custom, "Custom")
{
    qmlRegisterUncreatableType<CustomSettings>("QGroundControl.SettingsManager", 1, 0, "CustomSettings", "Reference only");
}

DECLARE_SETTINGSFACT(CustomSettings, takeoffAltitude)
DECLARE_SETTINGSFACT(CustomSettings, divisions)
DECLARE_SETTINGSFACT(CustomSettings, maxPulseStrength)
DECLARE_SETTINGSFACT(CustomSettings, k)
DECLARE_SETTINGSFACT(CustomSettings, falseAlarmPreset)
DECLARE_SETTINGSFACT(CustomSettings, falseAlarmProbability)
DECLARE_SETTINGSFACT(CustomSettings, gain)
DECLARE_SETTINGSFACT(CustomSettings, detectionFlightMode)
DECLARE_SETTINGSFACT(CustomSettings, antennaOffset)
DECLARE_SETTINGSFACT(CustomSettings, antennaType)
DECLARE_SETTINGSFACT(CustomSettings, rotationKWaitCount)
DECLARE_SETTINGSFACT(CustomSettings, useSNRForPulseStrength)
DECLARE_SETTINGSFACT(CustomSettings, allowMultiTagDetection)
DECLARE_SETTINGSFACT(CustomSettings, rotationType)
DECLARE_SETTINGSFACT(CustomSettings, detectionMode)
DECLARE_SETTINGSFACT(CustomSettings, pythonCrossRateCoalescingEnabled)
DECLARE_SETTINGSFACT(CustomSettings, detectionMargin)
DECLARE_SETTINGSFACT(CustomSettings, confidenceRatio)
DECLARE_SETTINGSFACT(CustomSettings, pythonK)
DECLARE_SETTINGSFACT(CustomSettings, pythonFalseAlarmPreset)
DECLARE_SETTINGSFACT(CustomSettings, pythonFalseAlarmProbability)
