...
public:
    ...
    enum BearingState {
        Pending,        // no BEARING_RESULT yet for this rotation
        NothingHeard,   // no bearing, no detections attributable to the tag
        Heard,          // no bearing, but the tag was detected: a lock that did not fit the
                        // pattern, or sub-lock hits at one frequency on several headings
    ...
    Q_PROPERTY(bool bearingHeard READ bearingHeard NOTIFY bearingChanged)
    ...
    // The controller attributed detections to the tag (n_valid_slices > 0 or a finite bearing)
    bool bearingHeard(void) const { return _bearingHeard; }
